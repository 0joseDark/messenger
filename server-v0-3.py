...
conexoes = {}  # username: (socket, endereco)
...
def atualizar_lista_usuarios():
    ...
    for user, (socket_cliente, endereco) in conexoes.items():
        host, porta = endereco
        lista_usuarios.insert(tk.END, f"{user} - {host}:{porta}\n")
...
def gerenciar_cliente(cliente, endereco):
    ...
    if username in usuarios and usuarios[username] == password:
        ...
        conexoes[username] = (cliente, endereco)  # Associa o socket e o endereço ao nome de usuário
        clientes.append(cliente)

        # Registra a conexão no log
        registrar_log(f"{username} ({host}:{porta}) entrou no chat.")
        atualizar_lista_usuarios()  # Atualiza a lista de usuários logados
        chat_log.insert(tk.END, f"{username} entrou no chat. (IP: {host}, Porta: {porta})\n")
    else:
    ...
    while True:
        try:
            ...
            if msg == "exit":
                registrar_log(f"{username} ({host}:{porta}) saiu do chat.")
                chat_log.insert(tk.END, f"{username} saiu do chat. (IP: {host}, Porta: {porta})\n")
                break
            ...
            if msg.startswith("PRIVATE:"):
                ...
                if destinatario in conexoes:
                    enviar_para_cliente(conexoes[destinatario][0], f"Mensagem privada de {username}: {mensagem}")
                ...
                enviar_para_todos(f"{username}: {msg}")  # Envia para todos os clientes
        except:
            clientes.remove(cliente)
            cliente.close()
